...
```{r}
#| label: fig-age
#| fig-cap: "Number of Households by Householder Age"
...
ggplot(hh1,
       aes(x = age,
           y = count,
           fill = forcats::fct_rev(living_arrangement))) +
...
```{r}
#| label: fig-tenure
#| fig-cap: "Number of Households by Tenure"
...
households own their homes. Only 1 in 5 boomers rent, compared to over half of millennials and over 90% of Gen Z households. Furthermore, among homeowner households, nearly one-third of boomers own their home without a mortgage or loan payments, compared to under one-tenth of millennial households.
...
```{r}
#| label: tbl-hud-inc-21
#| tbl-cap: "HUD Fair Market Rent (FMR) Income Limits, Virginia Beach-Norfolk-Newport News, 2021"

hud_21 <- read_excel("data/vchr_vb_data_clean.xlsx", sheet = "hud_21")
...
```{r}
#| label: tbl-hud-inc-23
#| tbl-cap: "HUD Fair Market Rent (FMR) Income Limits, Virginia Beach-Norfolk-Newport News, 2023"

hud_23 <- read_excel("data/vchr_vb_data_clean.xlsx", sheet = "hud_23") |> 
  mutate_at(vars(2:5), ~ currency(., digits = 0))
