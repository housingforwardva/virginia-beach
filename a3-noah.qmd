...
VCHR conducted an analysis of Naturally Occurring Affordable Housing (NOAH) on a City Council district basis for 1.5 persons (1-bedroom) and 4.5 persons (3-bedroom) with a 5% down payment. The assessment was focused on units suitable for households earning 80% or less / 100% or less of the Area Median Income (AMI). The results are presented in @tbl-units-affordable-cc-1br, @tbl-units-affordable-cc-3br, and @fig-cc-maps.
...
::: {#tbl-units-affordable-cc-1br}

![](img/units-affordable-cc-1br.png)

Number of Units by City Council District (1.5 person / 1 bedroom)

::: 

::: {#tbl-units-affordable-cc-3br}

![](img/units-affordable-cc-3br.png)

Number of Units by City Council District (4.5 persons / 3 bedrooms)
...
cc <- read_sf("data/cc_districts/City_Council_Districts.shp") |> 
  ...
  mutate(
    hh = case_when(
      str_detect(name, "_1") ~ "1.5-person",
      .default = "4.5-person"
    ),
    .before = 5
  ) |> 
  mutate(label = paste0("Affordable to ", hh, "\nHouseholds Below ", ami), .before = 6) |> 
  mutate(
    label = fct_relevel(
      label,
      "Affordable to 1.5-person\nHouseholds Below 80% AMI",
      "Affordable to 4.5-person\nHouseholds Below 80% AMI",
      "Affordable to 1.5-person\nHouseholds Below 100% AMI"
    )
  )

ggplot(filter(cc, type == "pct")) +
  ...
  scale_fill_gradientn(
    ...
    guide = guide_colorbar(ticks = FALSE, barheight = 23, barwidth = 1.5),
    limits = c(0.0001, .801),
    breaks = seq(0, .8, .1)
    ) +
  scale_x_continuous(limits = c(-76.229, -75.86), expand = c(0,0)) +
  scale_y_continuous(limits = c(36.65, 36.94), expand = c(0,0)) +
  labs(
    title = "Share of Affordable Units by Council District",
    subtitle = "By Household Size and AMI",
  ...
  theme(
    ...
    legend.text = element_text(size = 24, vjust = 0.5)
  )

ggsave("img/fig-cc-maps.png", width = 6.5, height = 7, units = "in", bg = "white")
...
Share of Affordable Units by Council District, Household Size, and AMI
...
VCHR used the income limits shown in @tbl-hud2022 and the associated maximum affordable rent (30% of the income limit divided into 12 monthly rent payments) shown in @tbl-units-size-afford, to categorize units by affordability level. The maximum affordable rent for a 1-person household is applied to studio apartments, the affordable rent for 1.5-person households is applied to 1-bedroom units, the affordable rent for a 3-person household is applied to 2-bedroom units, and the affordable rent for a 4.5-person household is applied to 3- and 4-bedroom units. VCHR adopted this convention from the rules applied by HUD to classify units for the Consolidated Housing Affordability Strategy (CHAS) data tabulations. VCHR used the maximum affordable rents in @tbl-units-size-afford to set ranges for property-wide average rents by unit type reported to CoStar.

::: {#tbl-units-size-afford}

![](img/units-size-afford.png)

Total Market Affordable Housing Units by Unit Size and Affordability Category

::: 

@tbl-units-size-afford shows the number of market-rate units that are affordable to households by income categories. More than half these units are 2-bedroom units, while 29% are 1-bedroom units. Approximately 16% are 3+ bedroom units, and studios make up only 2%. Nearly 90% of units are affordable to households with incomes 50%-100% of AMI, and few units are affordable to households with very-low or extremely low incomes, below 50% of AMI. This distribution pattern underscores the challenges faced by individuals and families with lower incomes in accessing affordable market-rate units, highlighting the need for more extensive affordable housing options for this population.
